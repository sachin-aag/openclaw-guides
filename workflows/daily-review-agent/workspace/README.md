...
This is your agent's mind on disk. **It starts empty on purpose** — OpenClaw scaffolds the canonical templates here on first run.
...
- `notes/` — daily logs and per-domain scratchpads
...
The daily-review skill needs a few extra rules in `SOUL.md` — things like "when the user types `#keep …`, append it to MEMORY.md" and "follow the daily-review skill spec when asked to review."
...
You can also paste it manually — the snippet is small and self-contained.
...
4. `cat ../skill-personality.snippet.md >> SOUL.md` — adds the daily-review rules
5. Open `http://localhost:3000` and start chatting

If `openclaw init` ever changes shape upstream, this README still works — we don't fight the upstream tool.
