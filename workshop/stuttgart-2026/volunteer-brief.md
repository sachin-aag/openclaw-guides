...
> **L1 = pair through it. L2 = park them on coral.inc.**
...
| **L2** | OS-specific install break, suspected OpenClaw bug, npm hangs forever on this network, mystery error after 15 min of poking | **Park them on the shared coral.inc instance** so they keep playing. Then come back to fix the local install in parallel — *but never block the participant on it.* |

The L2 move is the single most important rule. **Better that someone runs an agent in the cloud and feels victorious, than that they spend the whole session debugging Node.**
...
- **16:50** — Most of your zone has `npm run dev` running. The 2–3 that don't are the ones to focus on.
- **17:00** — *(facilitator pauses for a hand-raise check)* — anyone with their hand up gets you within 60 seconds.
- **17:30** — Everyone in your zone has either hit the success criterion or is sitting at the installation bar / on coral.inc.
